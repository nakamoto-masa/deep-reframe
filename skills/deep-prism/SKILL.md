...
**背景知識の参照:**
- ユーザーから「なぜAI可視性が重要か」「AI検索はどう動いているか」と聞かれたら、同ディレクトリの `background.md` を読んで回答する
- NGの項目について「なぜ今まで気づかなかったか」を説明する際も `background.md` の「なぜ気づかない」セクションを参照する
- 定量データやソースの詳細も `background.md` にまとめてある
